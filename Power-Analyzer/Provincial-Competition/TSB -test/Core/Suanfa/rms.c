...
#include "rms.h"
float cal_phase(unsigned int *input1){  //参数是AD采样的交流电压或电流值，待测信号的
...
float get_Vrms(unsigned int *digital_data,int k){ //
//基波频率为50Hz,参数是有至少500个样本点的数字信号数组

    float real_part = 0,imag_part = 0;            //实部虚部
    ...
    for(int i = 1;i < 152;i++){           //在半个周期中必出现零点
    ...
    float divide_k = k * 6.2831852 / N_DATA;              //k为基频次数此处取1

    for(int i = 0;i < N_DATA;i++){
        real_part += digital_data[i] * cos(divide_k * i); //计算实部
        imag_part -= digital_data[i] * sin(divide_k * i); //计算虚部
    }
