...
int cnt;
volatile uint32_t input_v[NUM_SAMPLES];
volatile uint32_t input_i[NUM_SAMPLES];//uint32_t数组 包实部虚部 

volatile uint16_t temp_amp[NUM_SAMPLES * 2];//作为存放幅度值的缓冲区
volatile float window_amp[NUM_SAMPLES * 2];//加窗之后的频谱图
//////////////////////////////////////////////////////////////////
...
//        }
    while (1){
        ...
        if(ADC_cnt0 == NUM_SAMPLES && ADC_cnt1 == NUM_SAMPLES){//开始计算
            ...
            //电流：
            for(uint16_t i = 0; i < NUM_SAMPLES ;i++)
                I_rams += pow(input_i[i],2) / NUM_SAMPLES;
            I_rams = sqrt(I_rams);//开方 得到有效值

            /*可加！！ 汉宁窗修正 和下面注释的的修正搭配可以提高精度（可能）*/
            /*当然可以替换成其他的加窗函数 最终多种加窗方案对比 得出最好的*/
            //hanning_window (input_i,NUM_SAMPLES);

            arm_cfft_q15(&arm_cfft_sR_q15_len512, (q15_t *) input_i, IFFTFLAG, BITREVERSE);//转化为实部 虚部的点集 其中 需要测出50HZ的电流的初相位
            arm_cmplx_mag_q15((q15_t *) input_i, (q15_t *) temp_amp, NUM_SAMPLES );//得到通过FFT得到  频谱
            ...
            /*测量电流相位*/
            /*不要管这个警告*/
            uint16_t * p =input_i + 5;//锁定50HZ对应的点
            uint16_t i_real = *p ,i_imag = *( p + 1);//实部虚部
            I_phase = atan((double)i_real / i_imag);//测得电流相位角（弧度制）

            /*测量各个电流分量的有效值*/
            for(u8 i = 0 ; i < 10 ; i++)//测量10个电流的有效值
                I_rams_arr[i] = (float)temp_amp[5*(i+1)] * 2.0 / (512 * 1.414213562373095);//为了写清楚 我这里写得难看 这句话就是把幅度转化为幅值 然后再转成有效值（除以根号二）
            
            /* 测量THD*/
            for(u8 i = 1; i < 10; i++)
                THD += pow((float)temp_amp[5*(i+1)] * 2.0 / 512 , 2);//累加幅度的平方
            THD = 100 * sqrt(THD) / pow(temp_amp[5]* 2 / 512 , 2) ;//得到THD的百分比

           /*测量电压相位*/
            /*不要管这个警告*/
            arm_cfft_q15(&arm_cfft_sR_q15_len512, (q15_t *) input_v, IFFTFLAG, BITREVERSE);//转化为实部 虚部的点集 其中 需要测出50HZ的电压的初相位
            p = input_v + 5;//锁定50HZ对应的点
            uint16_t v_real = *p ,v_imag = *( p + 1);//实部虚部
            V_phase = atan((double)v_real / v_imag);//测得电压相位角（弧度制）
