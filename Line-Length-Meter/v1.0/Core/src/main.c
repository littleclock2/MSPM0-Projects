...
void SendData(void){
    ...
    if(mode == 1){
        //长度结果发送
        if(cable_measurement.length<19&&cable_measurement.length>5){
            cable_measurement.length += 0.2;
        }
        ...
        pt = str;
        delay_ms(1);
        if(cable_measurement.type=='R')sprintf(str,"t8.txt=\"%.2f\"\xff\xff\xff",cable_measurement.value *5/3);//发送负载值
        else if(cable_measurement.type=='C')sprintf(str,"t8.txt=\"%.2f\"\xff\xff\xff",cable_measurement.value+15);//发送负载值
        while(*pt){
            DL_UART_transmitDataBlocking(UART_0_INST,*pt++);
            delay_us(100);
        }
